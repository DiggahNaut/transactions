...
class App extends Component {
  // Может не обязательно было класть transactions в state, но решил положить сюда.
    state = {
        transactions: [
          {
            "transaction": {
    ...
    render() {
      ...
      const paymentData = getPaymentData(this.state.transactions);
      ...
      return (
        <div className="container">
          <Tabs>
            ...
            <TabPanel>
              <TransactionsTable data={this.state.transactions} />
            </TabPanel>

            <TabPanel>
              <ProjectList transactionsData={this.state.transactions} />
            </TabPanel>
...
// Функция используется для получения массива платёжных систем с подсчётом всех совершённых транзакций по каждой отдельной системе.
function getPaymentData(props) {
  ...
  // Функция для сортировки данных по убыванию. Не знаю как по другому и как лучше сделать.
  // Эта функция на данный момент не универсальна и используется конкретно для данных по платёжным системам.
  function compare(a, b) {
    let comparison = 0;

    if (a.amountOfUse > b.amountOfUse) {
        comparison = -1;
    } else if (a.amountOfUse < b.amountOfUse) {
        comparison = 1;
    }
  ...
  return paymentMethods.sort(compare);
}
